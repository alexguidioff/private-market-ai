...
- How can each decision be represented by its **information state**: what is knowable at time *t*,
  reconstructed point-in-time, before the outcome is realised?
...
a **virtual-fund simulator** that lets us measure decision quality counterfactually — a natural
sequential-decision environment in which an acquisition policy can be learned and evaluated. On top
of this I formalise value-of-information estimators and an acquisition policy that selects the next
diligence step by expected marginal value per unit cost, benchmarked against the WI2026 pipeline as
a predictive baseline.
