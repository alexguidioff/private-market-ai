...
To make the research shareable and to seed PrivateBench (Chapter 11) without proprietary data:
`OpenVC + Crunchbase (where licensed) + GitHub + SEC + OpenAlex + GDELT + World Bank/OECD`.
This subset is enough to instantiate the ontology, build a small temporal graph, and run baseline
tasks — while keeping licensed data (PitchBook, Orbis) out of any public artifact.
