...
### 2.6 The Information State (the construct that makes VoI precise)
The most useful construct for the flagship paper. A decision is not made from "the data that
exists" but from an **information state** — *what the decision-maker knows at time t*. Formally,
the information state is the filtration available at t (the point-in-time reconstruction of §Feasibility
in `Research_Agenda.md`). This turns value of information into a clean object:

```
Decision(State_t)  →  acquire information (cost c)  →  State_t'  →  Decision(State_t')
VoI = E[ decision-quality(State_t') − decision-quality(State_t) ]  −  c
```

Two payoffs: (a) it makes **P1 (Value of Information)** a well-posed problem rather than a feature-
importance exercise; (b) it links directly to **P2 (Decision Quality)**, which evaluates a decision
*relative to its information state*, not to the eventual outcome.
